...
const DailyNotes: React.FC<DailyNotesProps> = ({ dailyNotes, setDailyNotes }) => {
  ...
  const handleSave = () => {
    ...
    if (!trimmed) {
      return alert("Escreva algo antes de salvar.");
    }

    let newNotes: Note[];

    if (editingId) {
      newNotes = dailyNotes.map((n) =>
        n.id === editingId
          ? { ...n, content: trimmed, date: selectedDate }
          : n
      );
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between bg-white dark:bg-zinc-900 p-4 rounded-3xl border border-gray-100 dark:border-zinc-800 shadow-sm">
        <h3 className="font-black text-gray-800 dark:text-zinc-100 uppercase tracking-widest text-xs">
          Diário de Notas
        </h3>
      ...
      {!isListView ? (
        <div className="space-y-4">
          <div className="bg-zinc-900 p-6 rounded-[40px] border-b-8 border-red-700 shadow-2xl min-h-[450px] flex flex-col">
            <input
              type="date"
              className="bg-zinc-800 text-white p-3 rounded-2xl mb-4 border-none outline-none font-bold"
              value={selectedDate}
            ...
            <div className="flex justify-center mt-4">
              <button
                ...
                className={`px-6 py-3 rounded-2xl font-black uppercase text-xs shadow-lg ${
                  isRecording
                    ? 'bg-red-500 text-white animate-pulse'
                    : 'bg-zinc-800 text-white border border-zinc-700'
                }`}
              >
                {isRecording ? '🎙️ Ouvindo...' : '🎙️ Voz'}
              </button>
            </div>
            ...
            <div className="grid grid-cols-2 gap-4 mt-6">
              <button
                onClick={handleSave}
                className={`${
                  editingId ? 'bg-blue-600' : 'bg-red-700'
                } text-white py-4 rounded-2xl font-black uppercase text-xs shadow-lg`}
              >
              ...
              <button
                ...
                className="py-4 rounded-2xl font-black uppercase text-xs bg-zinc-800 text-red-400 border border-zinc-700"
              >
          ...
          {dailyNotes.length > 0 ? (
            [...dailyNotes]
              ...
              .map((note) => (
                <div
                  key={note.id}
                  className="bg-zinc-900 p-6 rounded-[35px] border border-zinc-800 shadow-lg mb-4"
                >
                  <div className="flex justify-between items-center mb-4">
                    ...
                    <div className="flex space-x-2">
                      <button
                        ...
                        className="p-2 bg-zinc-800 rounded-xl text-blue-400 text-xs font-bold"
                      >
                        ✏️ Editar
                      </button>
                      ...
                      <button
                        ...
                        className="p-2 bg-zinc-800 rounded-xl text-red-400 text-xs"
                      >
                        🗑️
                      </button>
                    </div>
